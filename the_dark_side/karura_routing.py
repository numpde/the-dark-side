...
def load_route_asset(path: Path) -> dict:
    return validate_route_asset_document(load_required_json(path, label="route asset"), label="route asset")


def load_route_graph(path: Path) -> RouteGraph:
    payload = validate_route_graph_document(load_required_json(path, label="route graph"), label="route graph")
    ...
    for contig_payload in payload["contigs"]:
        contig = ContigRecord(
            ...
            tags={str(key): str(value) for key, value in contig_payload["tags"].items()},
        )
